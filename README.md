...
**Implementation status:** **19** wired to real service logic · **1** partial (bulk apartments reuses single-create) · **4** stub controllers · **7** smoke `/test` handlers.
...
| 15 | Apartments | `PATCH` | `/api/v1/apartments/update` | Clerk | Admin | ✅ Implemented | Update apartment details |
| 16 | Apartments | `GET` | `/api/v1/apartments/` | Clerk | Admin | ✅ Implemented | Paginated list of apartments for caller’s society |
...
| 19 | Visitors | `GET` | `/api/v1/visitors/` | Clerk | Admin | ✅ Implemented | List all visitors for society |
...
| 28 | Complaints | `GET` | `/api/v1/complaints/` | Clerk | Admin | ✅ Implemented | Paginated list of all complaints in society |
...
| 34 | Notices | `GET` | `/api/v1/notices/` | Clerk | Any authenticated | ✅ Implemented | Paginated list of notices for caller’s society |
...
- **`POST /api/v1/webhooks/clerk/persist-user-through-webhook`** — Clerk server-to-server webhook (raw JSON body + Svix headers). Creates a backend user when Clerk fires `user.created`.
...
- **`GET /api/v1/apartments/`** — Admin lists apartments for their society (paginated).
...
- **`GET /api/v1/complaints/`** — List all society complaints (paginated).
...
- **`GET /api/v1/notices/`** — Paginated list of notices for caller’s society.
